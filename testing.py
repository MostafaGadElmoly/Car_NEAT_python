...
def draw_info_table(info_window, generation, remain_cars, time_passed, cars_not_made_it):
    ...
    text_generation = font.render("Generation: " + str(generation), True, black)
    text_remain_cars = font.render("Cars Alive: " + str(remain_cars), True, black)
    text_time_passed = font.render("Time Passed: " + str(time_passed), True, black)
    text_cars_not_made_it = font.render("Cars Didn't Make It: " + str(cars_not_made_it), True, black)
...
def run_car(genomes, config):
    ...
    # Create info window
    info_window = pygame.display.set_mode((table_width, table_height))
    pygame.display.set_caption("Information Table")
    ...
    while True:
        ...
        # check if all car dead
        if remain_cars == 0:
            break

        # check if time passed with no change 
        counter += 1
        if counter == 30 * 40:
            break
        ...
        #draw_info_table(info_window, generation, remain_cars, pygame.time.get_ticks(), len(cars) - remain_cars)
        pygame.display.flip()
        clock.tick(0)
